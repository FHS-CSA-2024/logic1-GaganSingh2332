...
public class Logic1
{
    ...
    public boolean old35(int n) {
        ...
        // Return true if one is true and the other is false
    ...
    public boolean answerCell(boolean isMorning, boolean isMom, boolean isAsleep) {
        if (isAsleep) {
            return false; // If you're asleep, don't answer.
        }
        if (isMorning) {
            return isMom; // If it's morning, answer only if it's mom.
        }
    ...
    public boolean sodaParty(int sodas, boolean isWeekend) {
        if (isWeekend) {
            return sodas >= 40; // Successful if at least 40 sodas on weekend
        } else {
